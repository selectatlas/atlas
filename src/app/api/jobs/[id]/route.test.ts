...
describe('GET /api/jobs/[id]', () => {
  ...
  it('returns 403 when job belongs to another hirer', async () => {
    mockCreateClient.mockResolvedValue(makeClient({ id: 'user-1' }, { id: 'job-1', hirer_id: 'other-user' }))
    const res = await GET(new Request('http://localhost'), { params })
...
describe('PATCH /api/jobs/[id]', () => {
  ...
  it('returns 400 for invalid status value', async () => {
    mockCreateClient.mockResolvedValue(makeClient({ id: 'user-1' }, { id: 'job-1', hirer_id: 'user-1' }))
