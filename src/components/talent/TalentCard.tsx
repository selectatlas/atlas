...
export function TalentCard({ profile, matchScore, href, views, likes }: TalentCardProps) {
  ...
  const cardContent = (
    <Card className="group overflow-hidden border border-border/80 shadow-none transition-[border-color,transform] duration-[var(--duration-fast)] ease-[var(--ease-out)] hover:-translate-y-0.5 hover:border-primary/35">
        ...
        <div className="relative aspect-[4/3] overflow-hidden bg-muted">
          {profile.avatar_url ? (
            <Image
              ...
              fill
              className="object-cover transition-transform duration-[var(--duration-base)] ease-[var(--ease-out)] group-hover:scale-[1.03]"
      ...
      <CardContent className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h3 className="truncate text-sm font-semibold leading-tight">{profile.full_name}</h3>
            {profile.headline && (
              <p className="mt-1 line-clamp-1 text-xs text-muted-foreground">{profile.headline}</p>
            )}
            {location && (
        ...
        {topSkills.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-1.5">
            {topSkills.map(skill => (
              <Badge key={skill.id} variant={proficiencyVariant[skill.proficiency] ?? 'outline'} className="text-[11px]">
                {skill.skill}
              </Badge>
            ))}
        ...
        {(views !== undefined || likes !== undefined || profile.availability) && (
  ...
  if (href) return <Link href={href} className="block">{cardContent}</Link>
  return cardContent
}
  ...
  const content = (
    <Card className="group border border-border/80 p-3 shadow-none transition-[border-color,background-color,transform] duration-[var(--duration-fast)] ease-[var(--ease-out)] hover:-translate-y-0.5 hover:border-primary/35 hover:bg-card">
      <div className="flex items-center gap-3">
        <div className="relative size-12 shrink-0 overflow-hidden rounded-lg bg-muted">
          {profile.avatar_url ? (
            ...
            <div className="flex size-full items-center justify-center text-lg font-semibold text-muted-foreground/40">{profile.full_name[0]}</div>
          )}
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <h3 className="truncate text-sm font-semibold">{profile.full_name}</h3>
        ...
        <div className="hidden min-w-[170px] items-center gap-1.5 md:flex">
          {topSkills.map(skill => <Badge key={skill.id} variant="outline" className="text-[10px]">{skill.skill}</Badge>)}
          {profile.talent_skills.length > 2 && <span className="text-[11px] text-muted-foreground">+{profile.talent_skills.length - 2}</span>}
        </div>
        <div className="hidden w-24 shrink-0 text-right text-xs font-medium text-foreground/75 sm:block">{profile.rates?.split('/')[0].trim() || 'Rate on request'}</div>
        <div className="hidden w-24 shrink-0 text-right text-[11px] text-emerald-600 dark:text-emerald-400 sm:block">{profile.availability || '—'}</div>
      ...
      <div className="mt-2 flex items-center gap-3 pl-[60px] text-[11px] text-muted-foreground sm:hidden">
        ...
        {views !== undefined && <span className="inline-flex items-center gap-1"><Eye className="size-3" />{formatCount(views)}</span>}
        {likes !== undefined && <span className="inline-flex items-center gap-1"><Heart className="size-3" />{formatCount(likes)}</span>}
      </div>
    </Card>
  )
