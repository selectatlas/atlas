...
export function getProfileCompleteness(profile: TalentProfile, attributes?: TalentAttributesPayload) {
  ...
  const items: ProfileCompletenessItem[] = [
    ...
    { key: 'bio', label: 'Introduce your work', hint: 'A short story helps the right brief find you.', complete: Boolean(profile.bio?.trim()), weight: 10 },
    { key: 'location', label: 'Add your location', hint: 'Location helps hirers plan shoots and travel.', complete: Boolean(profile.city?.trim() && profile.country?.trim()), weight: 8 },
    ...
    { key: 'case-study', label: 'Add an outcome to a credit', hint: 'Credits with results become case studies hirers can trust.', complete: Boolean(profile.credits?.some(credit => credit.outcome?.trim())), weight: 4 },
  ]
